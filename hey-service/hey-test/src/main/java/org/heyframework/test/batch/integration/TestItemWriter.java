		...
		for (String item : items) {
			...
			if (!exceptionThrown && exceptionText != null && item.contains(exceptionText)) {
				...
				throw new RuntimeException(
						"A contrived exception thrown on the remote writer to demonstrate error handling over message queues for item " + item);
			}
			logger.error("********** [" + writerName + "] writing item: " + item);
